...
- Centralize an `encrypt-and-save` handler that runs convert→normalize→process before encryption, reuses the same processed URL for non-encrypted saves, and skips extra URL processing plus `pleaseParse` when saving encrypted links.
- Surface encryption errors (validation, prompt, API response) via notifications for command/context menu triggers and via popup status text for the shortcut button; do not attempt a save on encryption failure.
