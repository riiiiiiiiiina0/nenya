...
class BrightModeEnforcer {
  ...
  async loadPatterns() {
    ...
    try {
      const result = await chrome.storage.sync.get([WHITELIST_STORAGE_KEY, BLACKLIST_STORAGE_KEY]);
    ...
    try {
      ...
      return patterns.some(pattern => {
        try {
          ...
          console.warn('[BrightMode] Invalid pattern:', pattern.pattern, error);
          return false;
        }
      });
    } catch (error) {
  ...
  checkAndApplyBrightMode() {
    ...
    const isInWhitelist = this.matchesPattern(this.whitelistPatterns, currentUrl);
    const isInBlacklist = this.matchesPattern(this.blacklistPatterns, currentUrl);
    const isOSLightMode = window.matchMedia('(prefers-color-scheme: light)').matches;
    ...
    const shouldApplyBrightMode = isOSLightMode && isInWhitelist && !isInBlacklist;
  ...
  setupMutationObserver() {
    ...
    this.observer = new MutationObserver((mutations) => {
      ...
      if (shouldReapply) {
        console.log('[BrightMode] DOM changes detected, reapplying styles...');
  ...
  setupNavigationListeners() {
    ...
    this.handleVisibilityChange = () => {
      if (!document.hidden) {
        console.log('[BrightMode] Page became visible, checking bright mode...');
  ...
  hasBackgroundImage(element) {
    ...
    return Boolean(backgroundImage && backgroundImage !== 'none' && backgroundImage !== 'initial');
  }
